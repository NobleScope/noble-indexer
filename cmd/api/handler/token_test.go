...
// TestTokenBalanceListWithCursor tests cursor-based pagination
func (s *TokenHandlerTestSuite) TestTokenBalanceListWithCursor() {
	q := make(url.Values)
	q.Set("cursor", "AAAAAAAAAAU")
	q.Set("token_id", "0")
	...
	s.tbs.EXPECT().
		...
		DoAndReturn(func(ctx context.Context, filter storage.TokenBalanceListFilter) ([]storage.TokenBalance, error) {
			...
			s.Require().Equal(0, filter.Offset)
			s.Require().Equal(sdk.SortOrderAsc, filter.Sort)
			s.Require().EqualValues(5, filter.CursorID)
			s.Require().NotNil(filter.TokenId)
			s.Require().Equal("0", filter.TokenId.String())
			return []storage.TokenBalance{testTokenBalance1, testTokenBalance2}, nil
		}).
...
// TestTokenBalanceListCursorInResponse tests that cursor is returned in response
func (s *TokenHandlerTestSuite) TestTokenBalanceListCursorInResponse() {
	q := make(url.Values)
	q.Set("token_id", "0")
	...
	// cursor should encode the last element's id (testTokenBalance3.Id = 3)
	s.Require().NotEmpty(body.Cursor)
	decodedID, err := helpers.DecodeIDCursor(body.Cursor)
	s.Require().NoError(err)
	s.Require().EqualValues(testTokenBalance3.Id, decodedID)
}

// TestTokenBalanceListEmptyCursorOnEmptyResult tests that cursor is empty when no results
func (s *TokenHandlerTestSuite) TestTokenBalanceListEmptyCursorOnEmptyResult() {
	q := make(url.Values)
	q.Set("token_id", "0")
...
// TestTokenBalanceListInvalidCursor tests handling of invalid cursor
func (s *TokenHandlerTestSuite) TestTokenBalanceListInvalidCursor() {
	q := make(url.Values)
...
// TestTokenBalanceListCursorWithFilters tests cursor combined with address filter
func (s *TokenHandlerTestSuite) TestTokenBalanceListCursorWithFilters() {
	q := make(url.Values)
	q.Set("cursor", "AAAAAAAAAAI")
	q.Set("address", testAddressHex1.Hex())
	q.Set("token_id", "0")
	...
	s.tbs.EXPECT().
		...
		DoAndReturn(func(ctx context.Context, filter storage.TokenBalanceListFilter) ([]storage.TokenBalance, error) {
			...
			s.Require().NotNil(filter.AddressId)
			s.Require().EqualValues(1, *filter.AddressId)
			return []storage.TokenBalance{testTokenBalance3}, nil
		}).
		Times(1)
