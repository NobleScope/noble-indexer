...
func (t *Trace) Filter(ctx context.Context, filters storage.TraceListFilter) (traces []*storage.Trace, err error) {
	...
	err = t.DB().NewSelect().
		...
		ColumnExpr("contract.id AS contract__id, contract.address AS contract__address__address, contract.code AS contract__code, contract.verified as contract__verified").
